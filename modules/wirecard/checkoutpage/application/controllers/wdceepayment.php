...
class wdceepayment extends oxUBase
{
    ...
    protected static $_PLUGIN_VERSION = '2.8.3';
    ...
    protected function _wcpConfirmLogging($text)
    {
        ...
        if ($this->getConfig()->getConfigParam('bWcpLogConfirmations') == 1) {
    ...
    public function checkoutForm()
    {
        ...
        $oOrder = $this->_setOrderFolder($oOrder, $oConfig->getConfigParam('sWcpCheckoutFolder'));
        $oOrder->save();
        ...
        if ($initResult['result'] == 'OK') {
            ...
            $this->_aViewData['wcpPaymentUrl'] = $initResult['redirectUrl'];
            $this->_sThisTemplate = 'page/checkout/wcp_checkout_page.tpl';
        } else {
            $oSession = $this->getSession();
            $failureFolder = $this->getConfig()->getConfigParam('sWcpFailureFolder');
            $oOrder->cancelOrder();
            ...
            if ($request['checkoutType'] == 'IFRAME') {
                ...
                $this->_aViewData['wcpIFrameUrl'] = $sNextStep;
            } else {
    ...
    private function getSecret() {
        $oConfig = $this->getConfig();
        switch ($oConfig->getConfigParam('sWcpPluginMode')) {
            case 'Demo':
            ...
            case 'Live':
            default:
                return trim($oConfig->getConfigParam('sWcpSecret'));
        }
    }
    ...
    private function getCustomerId() {
        ...
        switch ($oConfig->getConfigParam('sWcpPluginMode')) {
            ...
            case 'Live':
            default:
                return trim($oConfig->getConfigParam('sWcpCustomerId'));
        }
    }
    ...
    private function getShopId() {
        ...
        switch ($oConfig->getConfigParam('sWcpPluginMode')) {
            ...
            case 'Live':
            default:
                return trim($oConfig->getConfigParam('sWcpShopId'));
        }
    }
    ...
    protected function _createWCPRequestArray($oOrder)
    {
        ...
        $customerStatementString = $oConfig->getConfigParam('sWcpShopName') . ' id:' . $orderReference;
        $customerStatementLength = ($paymenttype != 'POLI') ? self::$_CUSTOMER_STATEMENT_LENGTH : 9;

        if ($paymenttype == 'POLI') {
            $customerStatementString = substr($oConfig->getConfigParam('sWcpShopName'), 0, 9);
            ...
            $customerStatementString = substr($oConfig->getConfigParam('sWcpShopName'), 0,
        ...
        $request['serviceUrl'] = $oConfig->getConfigParam('sWcpServiceUrl');
        ...
        $request['imageUrl'] = $oConfig->getConfigParam('sWcpImageUrl');
        ...
        $request['maxRetries'] = $oConfig->getConfigParam('sWcpMaxRetries');
        $request['paymenttypeSortOrder'] = $oConfig->getConfigParam('sWcpPaymentTypeSortOrder');
        ...
        if ($oConfig->getConfigParam('bWcpSendAdditionalCustomerData') == 1) {
        ...
        if ($this->getConfig()->getConfigParam('bWcpDisableDeviceDetection') == 1) {
        ...
        if ($this->getConfig()->getConfigParam('bWcpUseLayout') == 1) {
            ...
            if ($oOrder->oxorder__oxtransstatus->value == 'PENDING') {
                ...
                if ($paymentState == 'CANCEL') {
                    $oSession->setVariable('wcp_payerrortext',
                        $oLang->translateString('wcp_payment_canceled', $iLangId));
                } elseif ($paymentState == 'FAILURE') {
                    $message = $oLang->translateString('wcp_payment_failure', $iLangId);
            ...
            if ($_POST['paymentState'] == 'SUCCESS') {
                ...
                if (strcasecmp($seed, $_POST['responseFingerprint']) == 0) {
                    if (!$this->_isPaid($oOrder)) {
                        ...
                        $successFolder = $this->getConfig()->getConfigParam('sWcpSuccessFolder');
                ...
                if ($_POST['paymentState'] == 'PENDING') {
                    ...
                    if (strcasecmp($seed, $_POST['responseFingerprint']) == 0) {
                        if (!$this->_isPaid($oOrder)) {
                            ...
                            $pendingFolder = $this->getConfig()->getConfigParam('sWcpPendingFolder');
                    ...
                    if (!$this->_isPaid($oOrder)) {
                        if ($_POST['paymentState'] == 'CANCEL') {
                            ...
                            $cancelFolder = $this->getConfig()->getConfigParam('sWcpCancelFolder');
    ...
    public static function getCheckoutType($sPaymentType)
    {
        ...
        if ($config->getConfigParam('bWcpDisableDeviceDetection') == 1) {
            $device = self::_getClientDevice();
            if ($device == 'tablet' || $device == 'smartphone') {
        ...
        return ($config->getConfigParam(sprintf('bWcp_%s_UseIframe',
