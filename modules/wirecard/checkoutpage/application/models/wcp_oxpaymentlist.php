...
class wcp_oxpaymentlist extends wcp_oxpaymentlist_parent
{
    ...
    protected function _isWCPInvoiceAvailable($oUser, $oBasket, $oOrder)
    {
        ...
        $oPayment = oxNew("wcp_payment");

        if (!$oPayment->wcpValidateCustomerAge($oUser)) {
            return false;
        }
        if (!(oxRegistry::getConfig()->getConfigParam('bWcpPayolutionAllowDifferingAddresses') && oxRegistry::getConfig()->getConfigParam('sInvoiceInstallmentProvider') == 'PAYOLUTION') && !$oPayment->wcpValidateAddresses($oUser,
                $oOrder)
        ) {
            return false;
        }
